...
if (isset($_POST['ajax_tambah'])) {
    ...
    $ok = mysqli_query($conn, "INSERT INTO supplier (nama_supplier, alamat, no_telp, email)
        VALUES ('$nama','$alamat','$telp','$email')");

    $id = mysqli_insert_id($conn);
    echo json_encode($ok
        ? ['success' => true, 'message' => 'Supplier berhasil ditambahkan!', 'id' => $id, 'nama' => htmlspecialchars($nama), 'alamat' => htmlspecialchars($alamat), 'telp' => htmlspecialchars($telp), 'email' => htmlspecialchars($email)]
...
if (isset($_POST['ajax_edit'])) {
    ...
    $ok = mysqli_query($conn, "UPDATE supplier SET nama_supplier='$nama', alamat='$alamat', no_telp='$telp', email='$email' WHERE id_supplier='$id'");
    echo json_encode($ok
...
if ($search !== '') {
    ...
    $where = "WHERE nama_supplier LIKE '%$s%' OR no_telp LIKE '%$s%' OR email LIKE '%$s%' OR alamat LIKE '%$s%'";
}

$query = mysqli_query($conn, "SELECT * FROM supplier $where ORDER BY id_supplier DESC");
$suppliers = [];
while ($r = mysqli_fetch_assoc($query)) $suppliers[] = $r;
?>
...
<body>
    ...
    <nav class="topnav">
        ...
        <span class="page-title">/ Supplier</span>
    ...
    <div class="app-body">
        ...
        <div class="main-content">
            <div class="page-header">
                <div>
                    <h2>Supplier</h2>
                    <p>Kelola data supplier apotek</p>
                </div>
                ...
                <form method="GET" id="ff">
                    <div class="table-toolbar">
                        <div class="search-box">
                            ...
                            <input type="text" name="q" placeholder="Cari nama / telepon / email..."
                                ...
                                <tr>
                                    <td colspan="6">
                                        <div class="empty-state"><i class="fas fa-truck"></i>
                                            ...
                                            <p style="font-size:12px;color:var(--muted)">Tambah supplier baru melalui tombol di atas</p>
                                        </div>
                                    </td>
                                </tr>
                            <?php else: foreach ($suppliers as $i => $s): ?>
                                <tr id="row-<?= $s['id_supplier'] ?>">
                                    <td><?= $i + 1 ?></td>
                                    ...
                                    <td>
                                        <div class="action-cell">
                                            <button class="btn-icon blue" title="Edit" onclick="openEdit(<?= $s['id_supplier'] ?>,'<?= addslashes(htmlspecialchars($s['nama_supplier'])) ?>','<?= addslashes(htmlspecialchars($s['alamat'])) ?>','<?= addslashes(htmlspecialchars($s['no_telp'])) ?>','<?= addslashes(htmlspecialchars($s['email'])) ?>')">
                                                <i class="fas fa-edit"></i>
                                            </button>
                                            <button class="btn-icon red" title="Hapus" onclick="confirmHapus(<?= $s['id_supplier'] ?>,'<?= addslashes(htmlspecialchars($s['nama_supplier'])) ?>')">
    ...
    <div class="modal-overlay" id="m-tambah">
        <div class="modal-box">
            ...
            <div class="fg">
                ...
                <input type="text" id="t-nama" class="fi" placeholder="Masukkan nama supplier">
            </div>
            <div class="fg">
                <label>Alamat</label>
                <input type="text" id="t-alamat" class="fi" placeholder="Alamat supplier">
            </div>
            <div class="fg">
                <label>No. Telp</label>
                <input type="text" id="t-telp" class="fi" placeholder="0812xxxx">
            </div>
            <div class="fg">
                <label>Email</label>
            ...
            fetch(BASE_URL, { method: 'POST', body: fd })
                ...
                .then(d => {
                    if (d.success) {
                        closeModal('m-tambah');
                        showToast(d.message);
                        insertRow(d.id, d.nama, d.alamat, d.telp, d.email);
                    } else showToast(d.message, true);
                })
                .catch(() => showToast('Koneksi gagal.', true));
        }

        function insertRow(id, nama, alamat, telp, email) {
            ...
            row.innerHTML = `
                ...
                <td>
                    <div class="action-cell">
                        <button class="btn-icon blue" title="Edit" onclick="openEdit(${id},'${nama.replace(/'/g, "\\'")}','${alamat.replace(/'/g, "\\'")}','${telp.replace(/'/g, "\\'")}','${email.replace(/'/g, "\\'")}')">
                            <i class="fas fa-edit"></i>
                        </button>
                        <button class="btn-icon red" title="Hapus" onclick="confirmHapus(${id},'${nama.replace(/'/g, "\\'")}')">
                            <i class="fas fa-trash"></i>
                        </button>
                    </div>
                </td>
                ...
                .then(d => {
                    if (d.success) {
                        ...
                        if (row) {
                            ...
                            cols[2].textContent = alamat;
                            cols[3].textContent = telp;
                            cols[4].textContent = email;
                        }
                    } else showToast(d.message, true);
                })
                .catch(() => showToast('Koneksi gagal.', true));
